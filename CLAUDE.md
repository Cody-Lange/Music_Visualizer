...
8. **Missing `mainImage` entry point** — The LLM sometimes generates code without any entry point (`void main()` or `void mainImage()`). The sanitizer has a multi-tier fallback: (a) rename `void main()` / `void main(void)` → `mainImage`, (b) fix near-miss signatures (wrong param names/qualifiers), (c) rename any `void foo(out vec4, in vec2)` → `mainImage`, (d) wrap bare `fragColor =` code in a synthesized `mainImage`, (e) **unconditional final guarantee** — if mainImage is STILL missing after all tiers, a minimal audio-reactive mainImage is appended. This ensures the static check never fails on "missing mainImage". Additionally, `_try_compile()` now returns `(error, sanitized_code)` so the retry pipeline always works with the sanitized version — previously, sanitizer fixes (like injected mainImage) were lost between retry iterations.
