...
8. **Missing `mainImage` entry point** — The LLM sometimes generates code without any entry point (`void main()` or `void mainImage()`). The sanitizer has a multi-tier fallback: (a) rename `void main()` / `void main(void)` → `mainImage`, (b) fix near-miss signatures (wrong param names/qualifiers), (c) rename any `void foo(out vec4, in vec2)` → `mainImage`, (d) wrap bare `fragColor =` code in a synthesized `mainImage`. If mainImage is STILL missing after all tiers, the code fails to compile naturally, which triggers the curated fallback shader selection (7 pre-tested visually complex shaders). Previously, a simple gradient was injected as an "unconditional guarantee" but this produced amorphous color blobs and prevented the curated fallbacks from being used. `_try_compile()` returns `(error, sanitized_code)` so the retry pipeline always works with the sanitized version. Both `_generate_and_validate()` (shader.py) and `render_shader_video()` (shader_render_service.py) fall back to `pick_fallback_shader()` after all LLM attempts are exhausted.
